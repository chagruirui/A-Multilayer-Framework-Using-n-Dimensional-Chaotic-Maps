...
for j=1:length(a)
    ...
    x(i+1)=cos(a(j)*z(i)+2*pi*(4/c*(sin(pi*x(i)))));
    y(i+1)=sin(b*x(i+1)-2*pi*(b*y(i)*(1-y(i))));
    z(i+1)=cos(c*y(i+1)+2*pi*(a(j)*acos(z(i))));
    end
    xn(:,j)=x(t + 1:n + t);
    yn(:,j)=y(t + 1:n + t);
    zn(:,j)=z(t + 1:n + t);
...
for j=1:length(b)
    ...
    y(i+1)=sin(b(j)*x(i+1)-2*pi*(b(j)*y(i)*(1-y(i))));
...
for j=1:length(c)
    ...
    x(i+1)=cos(a*z(i)+2*pi*(4/c(j)*(sin(pi*x(i)))));
    y(i+1)=sin(b*x(i+1)-2*pi*(b*y(i)*(1-y(i))));
    z(i+1)=cos(c(j)*y(i+1)+2*pi*(a*acos(z(i))));
