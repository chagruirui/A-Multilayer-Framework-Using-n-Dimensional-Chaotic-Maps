...
J0 = [                                                          -sin(a*y(1) + 2*b*pi*x(1) *(x(1)  - 1)^2)*(2*b*pi*(x(1)  - 1)^2 + 2*b*pi*x(1) *(2*x(1)  - 2)),                                                                     -a*sin(a*y(1)  + 2*b*pi*x(1) *(x(1)  - 1)^2);
-b*cos(b*cos(a*y(1)  + 2*b*pi*x(1) *(x(1)  - 1)^2) - 2*pi*(- y(1) ^2 + a))*sin(a*y(1)  + 2*b*pi*x(1) *(x(1)  - 1)^2)*(2*b*pi*(x(1)  - 1)^2 + 2*b*pi*x(1) *(2*x(1)  - 2)), cos(b*cos(a*y(1)  + 2*b*pi*x(1) *(x(1)  - 1)^2) - 2*pi*(- y(1) ^2 + a))*(4*pi*y(1)  - a*b*sin(a*y(1)  + 2*b*pi*x(1) *(x(1)  - 1)^2))];
	...
	y(n+1)=sin(b*x(n+1)-2*pi*(a-y(n)^2));
            ...
            %找出偏导矩阵
            % [-2 * a .* x 1; b 0]
xx=-sin(a*y(n) + 2*pi*b*x(n)*(x(n) - 1)^2)*(2*pi*b*(x(n) - 1)^2 + 2*b*x(n)*pi*(2*x(n) - 2));
xy= -a*sin(a*y(n) + 2*pi*b*x(n)*(x(n) - 1)^2);
yx=-b*cos(b*cos(a*y(n) + 2*b*pi*x(n)*(x(n) - 1)^2) - 2*pi*(- y(n)^2 + a))*sin(a*y(n) + 2*b*pi*x(n)*(x(n) - 1)^2)*(2*b*pi*(x(n) - 1)^2 + 2*b*pi*x(n)*(2*x(n) - 2));
yy=cos(b*cos(a*y(n) + 2*b*pi*x(n)*(x(n) - 1)^2) - 2*pi*(- y(n)^2 + a))*(4*pi*y(n) - a*b*sin(a*y(n) + 2*b*pi*x(n)*(x(n) - 1)^2));
